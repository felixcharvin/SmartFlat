          ...
          <tbody>
            <tr v-for="sensor in sensors">
              ...
              <td>
                ...
                <a v-if="sensor.status != 'off'" class="btn btn-danger" :disabled="sensor.type=='ultrasonic'" @click="switchStatus(sensor.type, sensor.pin, 0)">Off</a>
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    switchStatus: function(type, pin, status) {
      axios.post(URL.rootAPI+'/'+type, {status: status})
      .then(res => {
        this.getSensors()
      })
      .catch(e => {
